class DeathRate
  ...
  def self.ingest_from_api
    ...
    death_rate_api_url = "http://api.worldbank.org/countries/indicators/SP.DYN.CDRT.IN?format=#{format}&per_page=#{per_page}&date=#{date_range}"
    death_rate_api_uri = URI.parse(death_rate_api_url)
    res = Net::HTTP.get_response(death_rate_api_uri)
